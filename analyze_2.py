...
def main(args=None):
    ...
    with open(outputFolder + "\\common_words.txt",'w') as f:
        print("debug43")
        #f.write(f'{model.wv.doesnt_match("Argument case evt".split())}\n')
        print("debug44")
        f.write(f'{model.wv.most_similar("begin_method")}\n')
        print("debug45")
        f.write(f'{model.wv.similarity("if", "else")}\n')
        print("debug46")
...
def get_parser():
    ...
    parser.add_argument('--files_limit', action="store", dest="files_limit", type=int, default=50000)
